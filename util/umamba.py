...
class Umamba():
    def __init__(self, model: Literal['bot', 'enc'] = 'enc'):
        # self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        
        self.base_dir = Path('/ictstr01/groups/shared/users/lion.gleiter/organoid_sam/umamba_test/evaluation/segs')

        self.model = model
        if self.model == 'bot':
            self.mask_dir = self.base_dir / 'nnUNetTrainerUMambaBot'
        elif self.model == 'enc':
            self.mask_dir = self.base_dir / 'nnUNetTrainerUMambaEnc'
        else:
    ...
    def predict_cellsam(self, ds_name, image_idx, offset_x=0, offset_y=0, scale = None):

        mask_file = list(self.mask_dir.glob(f'*_{ds_name}/image_{image_idx:03d}.png'))[0]
    ...
    def forward_one_patch(self, ds_name, image_idx, offset_x, offset_y, scale=None):
        pred_scores_patch, pred_boxes_patch, pred_contours_patch = self.predict_cellsam(ds_name, image_idx, 
                                                                                        offset_x=offset_x, 
                                                                                        offset_y=offset_y, 
                                                                                        scale=scale)
    ...
    def forward(self, ds, image_idx, patch_size=1024, predict_masks=True, min_diameter=5):
        self.reset_image()
        ...
        boxes_patch, scores_patch, contours = \
            self.forward_one_patch(ds_name, image_idx,
                                   offset_x=0,
                                   offset_y=0)
