...
class DetectionHeadDataset(Dataset):
    def __init__(self, 
        ...
        if self.encoder_name.startswith('MicroSAM'):
            parent_dir: str = os.path.join(self.base_dir, "patch_embeddings2", self.encoder_name, self.data_split)
        else:
        ...
        for img_file, label_file in zip(self.image_files, self.label_files):
            ...
            if self.encoder_name.startswith('MicroSAM'):
                img_base_cleaned = img_base.replace("patch_embeddings2/" + self.encoder_name, "")
            else:
