...
class DetectionHeadDataset(Dataset):
    def __init__(self, 
        ...
        # Files for SAM embeddings
        embed_dir = self.base_dir / 'patched_data' / 'embeddings' / self.encoder_name / self.data_split

        embed_files = []
        for dataset in organoid_dirs:
            embed_files += list(embed_dir.glob(f'{dataset}/*/*.pt'))

        # Sort embed files
        def sorting_key(file: Path):
            im_dir = file.parent.as_posix()
            patch_number = int(file.stem.replace('images_patches_emb_', ''))
        ...
        for img_file, label_file in zip(self.image_files, self.label_files):
            # Get path infos
            img_patch_number = int(img_file.stem.replace('images_patches_emb_', ''))
            img_path_cleaned = img_file.parent.as_posix().replace(f'embeddings/{self.encoder_name}', '')
            label_path_cleaned = label_file.parent.as_posix().replace(f'patch_bbox_gt', '')

            img_path_cleaned = os.path.join(img_path_cleaned, f'patch_{img_patch_number}')
